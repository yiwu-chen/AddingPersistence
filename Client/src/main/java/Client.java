...
public class Client {
  private static boolean getRepeat = true;

  //private static String url = "http://localhost:8080/Server_war_exploded/textbody/wordcount";
  //private static String url = "http://52.73.40.7:8080/Server_war/textbody/wordcount";
  ...
  public static void main(String[] args) throws InterruptedException,
      ...
      public void run() {
        ...
        while (getRepeat) {
          ...
          try {
            ...
            BigDecimal res1 = api.getWordCount("the");
            BigDecimal res2 = api.getWordCount("distributed");
            BigDecimal res3 = api.getWordCount("system");
            BigDecimal res4 = api.getWordCount("building");
            BigDecimal res5 = api.getWordCount("great");
            BigDecimal res6 = api.getWordCount("today");
            BigDecimal res7 = api.getWordCount("thread");
            BigDecimal res8 = api.getWordCount("Barrier");
            BigDecimal res9 = api.getWordCount("Summary");
            BigDecimal res10 = api.getWordCount("Caching");
            endTime = System.currentTimeMillis();
            latency = endTime - startTime;
            getResponseTimes.add((int)latency);
            numGetRequests += 10;
        ...
        //Print out # requests send and the mean and max response time
        System.out.println("Number of GET requests: " + numGetRequests);
        System.out.println("Mean response time for GET requests: " + calculateMean(getResponseTimes, getResponseTimes.size()));
        System.out.println("Max response time for GET requests: " + calculateMaxResponseTime(getResponseTimes));
        getLatch.countDown();
      }
    });
    getThread.start();
    ...
    barrier.await();
    Client.getRepeat = false;

    //main() wait until getThread complete
    getLatch.await();
  ...
  private static double calculateMean(List<Integer> responseTimes, int length) {
    double sum = 0.0;
    for (int responseTime : responseTimes) {
      sum += responseTime;
    }
